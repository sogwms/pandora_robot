...
{
    volatile uint16_t idx_data, idx_bit;

    for (idx_data = 0; idx_data < len; idx_data++)
    {
        pb_recv[idx_data] = 0;
        for (idx_bit = 0x01; idx_bit < 0x0100; idx_bit <<= 1)
        {
            if (idx_bit & pb_send[idx_data])
                HAL_DO_H(); 
            else
                HAL_DO_L();
            HAL_CLK_H();
            ...
            HAL_CLK_H();
            if (HAL_GET_DI())
                pb_recv[idx_data] = idx_bit | pb_recv[idx_data];
        }
        _delay_us(16);
    }
}
